...
import { 
  MapPin, Plus, Trash2, Edit3, 
  SearchX, Loader2, Home,
  Map, Globe2, Hash, Navigation,
...
export const AddressSection: React.FC<AddressSectionProps> = ({ globalView = false }) => {
  const { user } = useAuth();
  const { t } = useLanguage();
  const [addresses, setAddresses] = useState<Address[]>([]);
  const [paginatedData, setPaginatedData] = useState<PaginatedResponse<Address> | null>(null);
    ...
    try {
      if (globalView) {
        const [addressesData, usersData] = await Promise.all([
          addressService.getAllAddresses(page, pageSize, includeInactive),
          userService.getAllUsers(1, 1000, true)
        ]);
        ...
        setPaginatedData(addressesData);
        ...
        if (Array.isArray(data)) {
          ...
          const paginated = data as unknown as PaginatedResponse<Address>;
          setAddresses(paginated.items || []);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (selectedAddress.id) {
        await addressService.updateAddress(selectedAddress.userId || user.id, selectedAddress.id, selectedAddress);
        toast.success(t("successUpdate"));
      } else {
        await addressService.createAddress(selectedAddress as Omit<Address, "id">);
  ...
  const handleDelete = async () => {
    ...
    try {
      await addressService.deleteAddress(addressToDelete.userId, addressToDelete.id);
  ...
  return (
    <div className="bg-white dark:bg-white/5 rounded-[2.5rem] border border-slate-200 dark:border-white/5 p-8 shadow-sm">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="text-xl font-black text-slate-800 dark:text-white uppercase tracking-tight">
              {globalView ? t("manageAddressesTitle") : t("navAddresses")}
            </h3>
            <p className="text-xs font-bold text-slate-400 dark:text-white/20 uppercase tracking-widest">
              {paginatedData ? paginatedData.totalItems : addresses.length} {t("addressRecords")}
            </p>
          </div>
        </div>
      ...
      {loading && (addresses.length === 0 && !paginatedData) ? (
        ...
        <div className="flex flex-col items-center justify-center py-12 text-center">
          ...
          <p className="text-sm font-bold text-slate-400 dark:text-white/20 uppercase tracking-[0.2em]">
            {globalView ? t("noAddressesFound") : t("noAddressesYet")}
          </p>
        </div>
      ) : (
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {addresses.map((address) => (
              <div key={address.id} className="group relative bg-slate-50/50 dark:bg-white/5 border border-slate-200/50 dark:border-white/5 rounded-3xl p-6 hover:border-zoco-lime hover:bg-white dark:hover:bg-white/10 transition-all shadow-sm hover:shadow-md">
                ...
                {globalView && (
                  <div className="flex items-center gap-2 text-[10px] font-black text-slate-400 dark:text-white/20 uppercase tracking-widest mb-2">
                    <UserIcon size={12} />
                    <span>{t("user")}: {userNames[address.userId] || address.userId.split("-")[0] + "..."}</span>
                  </div>
                )}
      ...
      <Modal 
        ...
        title={selectedAddress?.id ? t("edit") : t("add")}
      >
        <form onSubmit={handleSave} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              <Map className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300" size={16} />
              <input 
                type="text"
                required
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              <Globe2 className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300" size={16} />
              <input 
                type="text"
                required
      ...
      <ConfirmModal 
        ...
        title={t("delete")}
        message={`${t("deleteConfirm")} "${addressToDelete?.street}"?`}
      />
    </div>
  );
};
